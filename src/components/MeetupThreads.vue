    ...
    <nb-content>
      ...
      <view v-for="thread in threads" :key="thread._id">
        <nb-list-item avatar>
          ...
          <nb-right>
            <!-- Thread Date -->
            <nb-text note>{{thread.createAt | formatDate}}</nb-text>
          </nb-right>
        </nb-list-item>
        <!-- Iterate Posts Here -->
        <view class="post-container" v-for="post in thread.posts" :key="post._id">
          <nb-list-item avatar>
            <nb-left>
              <!-- Post User Avatar -->
            ...
            <nb-right>
              <nb-text note>{{post.updateAt | fromNow}}</nb-text>
            </nb-right>
          </nb-list-item>
        </view>
      </view>
